...
result=$(curl -X POST -u $USERNAME:$CURRENT_PASSWORD ${API_URL}/${NAMESPACE}/update-password  -o /dev/null -sw '%{http_code}' -H 'Content-Type: application/json' -d '{"new_password": "'$NEW_PASSWORD'"}');

if [ "$result" == "200" ]; 
then echo "Passwords Changed Successfully"; exit 0; 
else 
    echo "Failed. Status Code: $result"; 
    curl -X POST -u $USERNAME:$CURRENT_PASSWORD ${API_URL}/${NAMESPACE}/update-password -H 'Content-Type: application/json' -d '{"new_password": "$NEW_PASSWORD"}';
    exit 1; 
fi;
